...
class Scorer(Inferencer):
    ...
    def write_results(self):
        ...
        for entry in example_list:
            sorted_by_length = sorted(entry['ctxs_candidates'], key=lambda x: len(x["ctxs"]))
            prev_best_permutation = sorted_by_length[0]

            sorted_by_score = sorted(enumerate(sorted_by_length[1:]), key=lambda x: x[1]['score'])

            if len(sorted_by_score) == 0:
                entry["best_permutations"] = prev_best_permutation["ctxs"]
                entry.pop("ctxs_candidates")
                continue

            if prev_best_permutation['score'] > sorted_by_score[0][1]['score']:
                entry["best_permutations"] = sorted_by_score[0][1]["ctxs"]
            else:
                entry["best_permutations"] = prev_best_permutation["ctxs"]

            if "sorted_candidates" not in entry:
                entry["sorted_candidates"] = []
            if "score_gained" not in entry:
                entry["score_gained"] = []
            if "useless_candidates" not in entry:
                entry["useless_candidates"] = []

            entry["sorted_candidates"].append([i[1]['ctxs'][-1] for i in sorted_by_score])
            entry["score_gained"].append([prev_best_permutation['score']-i[1]['score'] for i in sorted_by_score])
            entry["useless_candidates"].extend([i[1]['ctxs'][-1] for i in sorted_by_score if i[1]['score'] >= prev_best_permutation['score']])
            entry.pop("ctxs_candidates")

            mrr += 1/([i[0] for i in sorted_by_score].index(0)+1)
...
@hydra.main(config_path="configs", config_name="advanced_scorer")
def main(cfg):
    logger.info(cfg)
    set_seed(43)
